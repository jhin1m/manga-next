...
export function RatingDisplay({
  ...
  return (
    <div className={cn('flex items-center gap-1', className)}>
        ...
        {[1, 2, 3, 4, 5].map(starIndex => {
          ...
          return (
            <div key={starIndex} className='relative'>
              ...
              <Star
                className={cn(sizeClasses[size], 'transition-colors text-muted-foreground/50')}
              />
      ...
      {/* Rating Value */}
      <span className={cn('font-medium', textSizeClasses[size])}>{getDisplayRating()}</span>

      {/* Rating Count */}
      {showCount && totalRatings !== undefined && (
        <span className={cn('text-muted-foreground', textSizeClasses[size])}>({totalRatings})</span>
      )}
    </div>
  );
}
