...
export class ChapterProcessor {
   ...
   */
  async processMany(chapters: StandardChapter[], comicId: number, options: ProcessorOptions = {}): Promise<void> {
   ...
   * @param options Tùy chọn xử lý
   */
  private async processPages(chapterId: number, pageUrls: string[], options: ProcessorOptions = {}): Promise<void> {
    // Xóa pages cũ
   ...
   */
  private async processPageImage(url: string, chapterId: number, pageNumber: number, options: ProcessorOptions): Promise<string> {
    ...
    try {
      ...
      console.error(`Error processing page image for chapter ${chapterId}, page ${pageNumber}:`, error);
