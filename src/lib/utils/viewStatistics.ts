...
export async function calculateViewStatistics(
  ...
  try {
    ...
    const baseWhere = entityType === 'comic' ? { comic_id: entityId } : { chapter_id: entityId };
...
export async function batchUpdateViewStatistics(
  ...
  for (let i = 0; i < entityIds.length; i += batchSize) {
    ...
    const promises = batch.map(entityId => updateEntityViewStatistics(entityType, entityId));
