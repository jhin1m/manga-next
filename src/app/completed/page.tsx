...
export const metadata: Metadata = {
  title: "Completed Manga | Manga Reader",
  description: "Browse completed manga series with all chapters available.",
};
...
async function fetchManga({ page = 1, limit = 20 }) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || ''}/api/manga?status=completed&page=${page}&limit=${limit}`,
...
export default async function CompletedPage({
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6">Completed Manga</h1>
