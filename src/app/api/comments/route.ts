...
export async function GET(request: Request) {
  try {
    ...
    if (!comic_id) {
      return NextResponse.json({ error: 'comic_id is required' }, { status: 400 });
    }
    ...
    const commentsWithUserData = comments.map(comment => ({
      ...comment,
      userLikeStatus:
        comment.CommentLikes?.[0]?.is_like === true
          ? ('like' as const)
      ...
      other_Comments: comment.other_Comments?.map(reply => ({
        ...reply,
        userLikeStatus:
          reply.CommentLikes?.[0]?.is_like === true
            ? ('like' as const)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: hasSpamKeywords ? 'Comment submitted for review' : 'Comment posted successfully',
        comment: {
          ...comment,
          userLikeStatus: null,
          repliesCount: 0,
