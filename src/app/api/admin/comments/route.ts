...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existingComments.length !== commentIds.length) {
      return NextResponse.json({ error: 'Some comments were not found' }, { status: 404 });
    }
    ...
    console.log(
      `Admin ${session.user.id} performed ${action} on comments: ${commentIds.join(', ')}${reason ? ` - Reason: ${reason}` : ''}`
    );
