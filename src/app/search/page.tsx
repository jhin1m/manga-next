      ...
      data: data.comics.map((comic: any) => ({
        ...
        latestChapter:
          comic.Chapters && comic.Chapters.length > 0
            ...
            : 'Updating',
        latestChapterSlug:
          comic.Chapters && comic.Chapters.length > 0 ? comic.Chapters[0].slug : '',
      ...
      <a href={`/manga/${item.slug}`} className='block'>
        ...
        <div className='p-3'>
          ...
          <div className='flex justify-between items-center text-xs text-muted-foreground'>
            <span>
              {item.chapterCount} {tManga('chapters')}
            </span>
            <span>
              {item.views} {tManga('views')}
            </span>
          </div>
              ...
              <div className='text-center py-12 bg-card rounded-lg shadow-sm'>
                {query ? (
                  <div>
                    ...
                    <p className='text-sm'>{tSearch('tryDifferentSearch')}</p>
                  </div>
                ) : (
                  <p className='text-muted-foreground'>{tSearch('enterSearchTerm')}</p>
                )}
              </div>
            )}
          </div>
      ...
      <div className='container mx-auto px-4 py-8'>
        ...
        <div className='bg-red-50 border border-red-200 rounded-md p-4 mb-6'>
          <h2 className='text-lg font-semibold text-red-700 mb-2'>
            {tSearch('errorLoadingResults')}
          </h2>
          <p className='text-red-600 mb-2'>
            {error instanceof Error ? error.message : tCommon('error')}
          </p>
          <p className='text-sm text-muted-foreground'>{tSearch('checkConnection')}</p>
        </div>
      </div>
    );
  }
