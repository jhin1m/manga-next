...
const checkProcessStatus = () => {
  return new Promise((resolve) => {
    try {
      ...
      // Check thresholds
      const warnings = [];
      if (memory > CONFIG.MEMORY_THRESHOLD * 10) { // Assuming 1GB limit
...
const runHealthCheck = async () => {
  ...
  const isHealthy = results.process.status === 'healthy' && 
  ...
  // Log summary
  info(`Overall Health: ${results.overall.status.toUpperCase()} (Score: ${results.overall.score}/100)`);
